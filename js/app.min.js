document.addEventListener("DOMContentLoaded", () => {
    ...
    // ================= FAST ORDER =================
    const quickOrderBtns = document.querySelectorAll(".item-catalog__btn:not(.btn-buy)");

    quickOrderBtns.forEach((btn) => {
        btn.addEventListener("click", (e) => {
            e.preventDefault();
            const catalogItem = btn.closest(".item-catalog");
            if (!catalogItem || !orderPopup) return;

            const titleEl = catalogItem.querySelector(".item-catalog__title");
            const priceEl = catalogItem.querySelector(".item-catalog__price span");
            const imgEl = catalogItem.querySelector(".item-catalog__img img");
            ...
            const product = [
                {
                    ...
                    price: parseFloat(priceEl.textContent.replace(/\s/g, "")),
                    quantity: 1,
                    image: imgEl.src,
                },
            ];

            renderOrderPopup(product);
            orderPopup.classList.add("open");
        });
    });
});
